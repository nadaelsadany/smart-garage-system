...
I2C_Config_t  Global_I2C_Config1 ;
I2C_Config_t  Global_I2C_Config2 ;
...
void MCAL_I2C_Init(I2C_Typedef * I2Cx ,I2C_Config_t * I2C_Config)
{
	...
	if (I2Cx == I2C1)
	{
		Global_I2C_Config1 = * I2C_Config ;
		Global_I2C_Config[I2C1_INDEX]=&Global_I2C_Config1;
		RCC_I2C1_CLK_EN();
	}
	else if (I2Cx == I2C2)
	{
		Global_I2C_Config2 = * I2C_Config ;
		Global_I2C_Config[I2C2_INDEX]=&Global_I2C_Config2;
		RCC_I2C2_CLK_EN();
	}
	...
	if (I2C_Config->I2C_Mode == I2C_Mode_I2C)
	{
		...
		Tmpreg &=~(I2C_CR2_FREQ); //Clear the first 6bits in CR2 register(FREQ[5:0]
		...
		Tmpreg = I2Cx->I2C_CR1;
		Tmpreg |= (uint16_t)(I2C_Config->Stretching |I2C_Config->I2C_ACK_Control | I2C_Config->I2C_Mode | I2C_Config->I2C_General_Call);
		I2Cx->I2C_CR1 = Tmpreg ;
		...
		{
			Tmpreg |=(I2C_OAR2_ENDUAL); //Enable Dual Address
			/*Set Secondary Address*/
			Tmpreg |=(I2C_Config->I2C_Slave_Address.Secondary_Slave_Address<<1);//bits 7:1 of address in dual addressing mode
			//Write to
 ...
 * @brief -                 -sending data by I2C
 * @param [in] -            -I2Cx : where x can be (1,2 depending on device used)
 * @param [in] -            -DevAddr : the address of slave
 * @param [in] -            - DataOut : address of register which contains data
 * @param [in] -            -DataLen : lenth of the data to transmit
 * @param [in] -            -Stop : to choose you want to stop transmitting or not
 * @param [in] -            -Start : to decide if you want a new start or not (@second start when you send dada on selected address )
 * @retval -                -None
 * note -                   -None
 */
/**================================================================= */
...
/**=================================================================
 * @Fn -                    -Generate_Start
 * @brief -                 -generate the start bit
 * @param [in] -            -I2Cx : where x can be (1,2 depending on device used)
 * @param [in] -            -NewState : boolean to choose enable or disable
 * @param [in] -            -State : to decide if you want a new start or not (@second start when you send dada on selected address )
 * @retval -                -None
 * note -                   -None
 */
/**================================================================= */
